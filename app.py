...
# =========================
    ...
    # train_test_split can fail for some targets if classes are extremely imbalanced or too few.
    # We'll try stratified split when possible, otherwise fall back to non-stratified.
    def _safe_split(X, y, test_size=0.2, random_state=42):
        ...
        try:
            return train_test_split(X2, y2, test_size=test_size, random_state=random_state, stratify=stratify_arg)
        except ValueError:
            return train_test_split(X2, y2, test_size=test_size, random_state=random_state, stratify=None)

    X_train, X_test, y_train_type, y_test_type = _safe_split(X_model, y_type, test_size=0.2, random_state=42)
    _, _, y_train_level, y_test_level = _safe_split(X_model, y_level, test_size=0.2, random_state=42)
