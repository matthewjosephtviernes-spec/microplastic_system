...
<style>
    ...
    /* Sidebar styling (may vary per Streamlit version) */
    ...
    with st.sidebar:
        ...
        with col1:
            st.metric(
                ...
                delta=f"+{max(0, len(st.session_state.items) - 5)}"
                if len(st.session_state.items) > 5 else None
            )
        with col2:
            st.metric("App Visits", st.session_state.app_visits)
        
        if st.session_state.items:
            completed = len([i for i in st.session_state.items if i.get('status') == 'Completed'])
            in_progress = len([i for i in st.session_state.items if i.get('status') == 'In Progress'])
            pending = len([i for i in st.session_state.items if i.get('status') == 'Pending'])
        ...
        # Export from sidebar: quick CSV of all
        if st.session_state.items:
            csv_all = pd.DataFrame(st.session_state.items).to_csv(index=False)
            st.download_button(
                "📥 Export All (CSV)",
                data=csv_all,
        ...
        if st.button("🧹 Clear Filters (UI hint only)", use_container_width=True):
            st.info("Filters can be adjusted on the 'View All' page.")
        ...
        current_theme = st.selectbox(
            ...
            if st.session_state.theme in ["Light", "Dark", "Blue"] else 0
        )
        ...
        if show_debug:
            ...
            st.write(f"Items in memory: {len(st.session_state.items)}")
            st.write(f"Next ID: {st.session_state.counter}")
            st.write(f"Session visits: {st.session_state.app_visits}")
...
def page_dashboard():
    ...
    with col3:
        st.markdown('<div class="metric-card">', unsafe_allow_html=True)
        avg_priority = np.mean([i.get('priority', 0) for i in st.session_state.items]) if st.session_state.items else 0
    ...
    with col4:
        ...
        overdue = len([
            ...
            if 'due_date' in i and datetime.strptime(i['due_date'], "%Y-%m-%d") < datetime.now()
        ])
...
def page_add_item():
    ...
    with col1:
        with st.form("add_item_form", clear_on_submit=False):
            ...
            with col_c:
                priority = st.slider("Priority", 1, 5, 3, help="1 = Low, 5 = Critical")
            with col_d:
...
def page_view_all():
    ...
    st.dataframe(
        display_df.style.background_gradient(
            subset=['priority'], cmap='RdYlGn_r'
        ).bar(
            subset=['progress'], color='#5DADE2'
        ),
        use_container_width=True,
        height=400
    )
    ...
    with col_action2:
        ...
        if st.button("🗑️ Delete Selected", use_container_width=True) and items_to_delete:
            st.session_state.items = [
    ...
    with col_action3:
        ...
        if st.button("🔄 Apply to Filtered", use_container_width=True):
            ...
            st.success(f"Updated {len(filtered_df)} items to '{new_status}'!")
            time.sleep(1)
            st.rerun()
...
def page_analytics():
    ...
    with col4:
        overdue = len([
            item for item in st.session_state.items
            if 'due_date' in item and datetime.strptime(item['due_date'], "%Y-%m-%d") < datetime.now()
        ])
    ...
    with export_col1:
        summary = {
            ...
            'completion_rate': float((df['status'] == 'Completed').mean() * 100),
...
def page_settings():
    ...
    with setting_tab1:
        ...
        if st.button("💾 Save General Settings", type="primary"):
            st.success("General settings saved successfully! (Demo only, not persisted)")
    ...
    with setting_tab2:
        ...
        with col_data1:
            ...
            if uploaded_backup:
                try:
                    backup_loaded = json.load(uploaded_backup)
                    st.info("Backup file loaded. Click button below to restore.")
                    if st.button("🔄 Restore Backup", type="secondary", use_container_width=True):
                        st.session_state.items = backup_loaded.get('items', [])
                        st.session_state.counter = backup_loaded.get('counter', 1)
                        st.session_state.app_visits = backup_loaded.get('app_visits', 0)
        ...
        with col_data2:
            ...
            if st.button("🗑️ Clear All Data", type="secondary", use_container_width=True):
                if clear_all and confirm1 and confirm2:
                    ...
                    st.warning("Please check all confirmation boxes before deleting.")
    ...
    with setting_tab3:
        ...
        if st.button("🎨 Apply Appearance Settings", type="primary"):
            st.success("Appearance settings applied! (Demo only, not persisted)")
